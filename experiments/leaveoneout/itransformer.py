...
from NeuroMamba.models.iTransformer import iTransformer
...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs-1:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/Desktop/DeepScore/weights/loo/itransformer/{fold}.pth'))
        
    return None

def evaluate(testloader, fold):
    model = iTransformer(seq_len=seq_len, d_model=d_model, dropout=dropout, mlp_factor=mlp_factor, n_heads=n_heads, e_layers=e_layers, score_amount=score_amount).to("cuda")
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/Desktop/DeepScore/weights/loo/itransformer/{fold}.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
