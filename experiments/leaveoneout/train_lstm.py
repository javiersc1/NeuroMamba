...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [4, 9, 19, 24]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/lstm/{fold}_{epoch+1}.pth'))
        #if val_loss < best_val:
            #best_val = val_loss
            #print("low epoch at ", epoch, " loss: ", best_val)
            #print("True Scores: ", np.array(totalScores).flatten())
            #print("Predicted Scores: ", np.array(totalPredictedScores).flatten())
            #torch.save(model.state_dict(), os.path.join(f'/home/javier/Desktop/DeepScore/weights/loo/neuromamba/{fold}.pth'))
