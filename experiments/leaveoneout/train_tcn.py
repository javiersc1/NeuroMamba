...
from NeuroMamba.models.TCN import TemporalConvNet
...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [4, 9, epochs-1]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/tcn/{fold}_{epoch+1}.pth'))
    
    return None

def evaluate(testloader, fold):
    model = TemporalConvNet(num_inputs=num_inputs, num_channels=num_channels, kernel_size=kernel_size, dropout=dropout).to("cuda")
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/loo/tcn/{fold}_{epochs}.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
...
def leaveoneout(files):
    ...
    for train_index, test_index in tqdm(loo.split(files), total=loo.get_n_splits(files), desc="LOO CV"):
        ...
        model = TemporalConvNet(num_inputs=num_inputs, num_channels=num_channels, kernel_size=kernel_size, dropout=dropout, score_amount=score_amount).to("cuda")
        # train model
