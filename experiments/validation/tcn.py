...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [epochs-1]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/tcn/val_{fold}.pth'))
...
def evaluate(testloader, fold):
    ...
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/loo/tcn/val_{fold}.pth')))
    model.eval()
    predicted = []
    true = []
