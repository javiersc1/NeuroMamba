...
def finetune(model, trainloader, optimizer, scheduler, epochs=100):
    ...
    for epoch in range(epochs):
        ...
        if epoch==epochs-1:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/neuromamba/singleshot.pth'))
...
def evaluate(testloader):
    ...
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/loo/neuromamba/singleshot.pth')))
    model.eval()
    realScores = []
    predictedScores = []
...
if __name__ == "__main__":
    ...
    for cls in np.unique(labels):
        ...
        selected_indices = cls_indices[:1]
