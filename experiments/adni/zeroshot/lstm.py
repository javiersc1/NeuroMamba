...
def train(model, trainloader, optimizer, scheduler, epochs=100):
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs-1:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/lstm/all.pth'))
...
def evaluate(testloader):
    ...
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/loo/lstm/all.pth')))
    model.eval()
    realScores = []
    predictedScores = []
