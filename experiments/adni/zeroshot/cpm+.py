...
from NeuroMamba.models.CPM import CPM
...
if __name__ == "__main__":
    ...
    mode = "home"
    if mode == "home":
        experiment_dir = "/home/javier/Desktop/NeuroMamba/experiments/adni/zeroshot/"
    ...
    # params
    workers = 8
    adni_df = adni_import("/home/javier/Desktop/NeuroMamba/adni.csv")
    score_amount = 1
    modescore = "mocaz"
    # model params
    C = 0.1
    ...
    madc_files = np.array(get_files(madc_path, madc_score, type="rest", subject_class="remove_unknown"))
    ...
    #X_train, y_train, masks_madc = CPM(madc_loader, score_amount=score_amount, mode=modescore, edge_type="positive", thresh=0.01)
    # # save files
    # np.save('/home/javier/weights/cpm+_subjectFeatures_madc.npy', X_train)
    # np.save('/home/javier/weights/cpm+_Scores_madc.npy', y_train)
    # np.save('/home/javier/weights/cpm+_masks_madc.npy', masks_madc)
    # load files
    X_train = np.load('/home/javier/weights/cpm+_subjectFeatures_madc.npy')
    y_train = np.load('/home/javier/weights/cpm+_Scores_madc.npy')
    ...
    adni_dataset = ADNI_LOADER(adni_files, transforms=None, database=adni_df, file_mode="train")
    adni_loader = DataLoader(adni_dataset, batch_size=1, shuffle=False, drop_last=False, num_workers=workers)
    #X_test, y_test = CPM_ADNI(adni_loader, score_amount=score_amount, mode=modescore, file_mode="all")
    #X_test, y_test, masks_adni = CPM_ADNI(adni_loader, score_amount=score_amount, mode=modescore, file_mode="train", edge_type="positive", thresh=0.01)
    # # save files
    # np.save('/home/javier/weights/cpm+_subjectFeatures_adni.npy', X_test)
    # np.save('/home/javier/weights/cpm+_Scores_adni.npy', y_test)
    # np.save('/home/javier/weights/cpm+_masks_adni.npy', masks_adni)
    # load files
    X_test = np.load('/home/javier/weights/cpm+_subjectFeatures_adni.npy')
    y_test = np.load('/home/javier/weights/cpm+_Scores_adni.npy')
