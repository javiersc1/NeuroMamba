...
def train(model, trainloader, optimizer, scheduler, epochs=100):
    ...
    for epoch in range(epochs):
        ...
        for idx, (data,info) in enumerate(trainloader):
            scores = mapScores(info, num=score_amount_train, mode=score_mode).to("cuda", dtype=torch.float32)
        ...
        if epoch == epochs-1:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/neuromamba/all.pth'))
