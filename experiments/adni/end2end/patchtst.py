...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [9, epochs-1]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/adni/patchtst/{fold}_{epoch+1}.pth'))
    
    return None

def evaluate(testloader, fold):
    model = PatchTST(n_layers = n_layers, n_heads=n_heads, patch_length=patch_length, patch_stride=patch_stride, d_model = d_model, ffn_dim=ffn_dim, dropout=dropout, score_amount=score_amount, time_samples=time_samples).to("cuda")
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/adni/patchtst/{fold}_{epochs}.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
