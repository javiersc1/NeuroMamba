...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [9, epochs-1]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/adni/lstm/{fold}_{epoch+1}.pth'))
...
def evaluate(testloader, fold):
    ...
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/adni/lstm/{fold}_{epochs}.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
...
if __name__ == "__main__":
    ...
    results_df.to_csv(os.path.join(experiment_dir, 'lstm.csv'), index=False)