...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [9, epochs-1]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/adni/tcn/{fold}_{epoch+1}.pth'))
...
def evaluate(testloader, fold):
    ...
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/adni/tcn/{fold}_{epochs}.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
