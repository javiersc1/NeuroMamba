...
from NeuroMamba.models.NeuroMamba import MambaModel
from sklearn.kernel_ridge import KernelRidge
from tqdm import tqdm

def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    #model.load_state_dict(torch.load(os.path.join(f'/home/javier/Desktop/DeepScore/weights/loo/mamba/{fold}.pth')))
    best_val = 100000
    for epoch in range(epochs):
        ...
        if epoch in [9, 24]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/mamba/{fold}_{epoch+1}.pth'))
    
    return None

def evaluate(testloader, fold):
    model = MambaModel(n_layers = n_layers, state_dim = state_dim, num_variables=272, score_amount=score_amount).to("cuda")
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/loo/mamba/{fold}_25.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
...
if __name__ == "__main__":
    ...
    mode = "home"
    if mode == "home":
        experiment_dir = "/home/javier/Desktop/DeepScore/experiments/leaveoneout_ablation/"
        ...
        experiment_dir = "/home/javiersc/DeepScore/experiments/leaveoneout_ablation/"
