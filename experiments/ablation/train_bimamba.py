...
from NeuroMamba.models.NeuroMamba import BiMambaModel
...
def train(model, trainloader, valloader, optimizer, scheduler, epochs=100, fold=0):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [9, 24]:
            torch.save(model.state_dict(), os.path.join(f'/home/javier/weights/loo/bimamba/{fold}_{epoch+1}.pth'))
    
    return None

def evaluate(testloader, fold):
    model = BiMambaModel(n_layers = n_layers, state_dim = state_dim, num_variables=272, score_amount=score_amount).to("cuda")
    model.load_state_dict(torch.load(os.path.join(f'/home/javier/weights/loo/bimamba/{fold}_25.pth')))
    model.eval()
    realScores = 0.
    predictedScores = 0.
